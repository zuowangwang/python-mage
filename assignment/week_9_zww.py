...
# 请同学们至少完成腾讯课堂如下学习章节：
# 【一：章节学习】
# 第27节  Python面向对象(一)	01-面向对象概述
# 	02-类定义和初始化
# 	03-实例变量和类变量及实例属性访问顺序
# 	04-类方法和静态方法装饰器
# 	05-访问控制、猴子补丁、属性装饰器、对象销毁
# 第28节  Python面向对象(二)	01-习题随机数生成、坐标打印、车辆管理类
# 	02-习题温度转换和购物车实现
# 	03-单继承
# 第29节  Python面向对象(三)	01-二分bisect
# 	02-古典类新式类和MRO
# 	03-Mixin
# 	04-PEP
# 【二：本周作业】请同学们于下周天晚上10点前，将作业上传至GitHub；
# 优化atm系统，要求使用数据库，本次作业要求，根据上次自定义的表结构，所有用户数据都存在表里面，  增删改查都从数据库里面调用，注意：当要查询的数据为空的时候，需要抛出一个异常不存在的异常(NoExist)。
