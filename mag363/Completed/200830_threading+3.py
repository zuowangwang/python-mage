...
def window(q:Queue, handler, width:int, interval:int):
    ...
    while True:
        ...
        if (current - start).total_seconds() >= interval:  #要注意时间问题，时间5秒时间能有很多数据，如果时间一直改变，会一直生成数据
            ret = handler(buffer)
            # print('{:.2f}'.format(ret)) #因为不是平均值了所以不能用这个
            print(ret)
...
def dispatcher(src):
    ...
    def run():
        ...
        while True:
            ...
            #         print(allbrowsers)
            #         newdict = {}
            #         for (k,ver),val in allbrowsers.items():
            #             newdict[k] = newdict.get(k, 0) + val
            #
            #         from pandas import Series  #对有序列的值进行文档排序
...
if __name__ == "__main__":
    ...
    path = r'.\mag363\configurefile\test.log' #实际日志的数据源
    reg, run = dispatcher(load(path))
    reg(status_handler, 10, 10)
